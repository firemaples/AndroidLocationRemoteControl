var div_mapDiv = $("#div_mapDiv"), tv_position = $("#tv_position"), 
    tv_search = $('#pac-input'), div_searchBox = $("#div_searchBox");
...
function loadGoogleMapApi(apiKey, callback) {
    ...
    div_mapBody.show();
    if (!mapLoaded) {
        $.getScript("https://maps.googleapis.com/maps/api/js?libraries=places&callback=mapInit&key=" + apiKey)
            .done(function (evt) {
                mapLoaded = true;
                callback();
            })
...
function mapInit() {
    ...
    $(".background", div_searchBox).stop().animate({opacity:0});
